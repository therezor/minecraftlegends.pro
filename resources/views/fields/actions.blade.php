...
<div class="dropdown-menu fs-sm">
    ...
    @if($field->hasUrl('edit'))
        <a href="{{ $field->url('show', $value) }}" title="{{ trans('crud.edit') }}" class="dropdown-item fs-sm fw-medium">
            <i class="fa fa-fw fa-pencil-alt"></i> {{ trans('crud.edit') }}
        </a>
    @endif

    @if($field->hasUrl('destroy'))
        <a href="{{ $field->url('show', $value) }}" data-confirm-message="{{ trans('crud.confirmation') }}" title="{{ trans('crud.destroy') }}" data-method="delete" class="dropdown-item fs-sm fw-medium">
