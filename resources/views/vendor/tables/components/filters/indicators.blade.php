        ...
        <div class="flex flex-1 flex-wrap items-center gap-x-2 gap-y-1">
            ...
            @foreach ($indicators as $filter => $filterIndicators)
                @foreach ($filterIndicators as $field => $indicator)
                    ...
                    <span
                        @class([
                            'filament-tables-filter-indicator min-h-6 inline-flex items-center justify-center whitespace-normal rounded bg-gray-500/10 px-2 py-0.5 text-xs font-medium tracking-tight text-gray-700',
